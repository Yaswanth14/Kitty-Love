  ...
  try {
    ...
    switch (true) {
      ...
      case !gender:
        return res
          .status(500)
          .send({ success: false, message: "Gender is required" });
...
module.exports.getProfiles = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error in getting profiles" });
  }
};
...
module.exports.getProfilePhoto = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ success: false, message: "Error while getting profile photo" });
  }
};
